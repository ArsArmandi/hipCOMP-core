...
public:
   ...
   * @param mask Lane mask with Nth bit set to 1 if and only if the Nth work
   * item in the calling wavefront belongs to the cooperative group.
   */
  ...
  /**
   * @brief Evaluate predicate for all work items in the cooperative group and
   * returns non-zero if and only if predicate evaluates to non-zero for any of
   * them.
   * @param pred The predicate to evaluate.
   * @return True if the predicate evaluates to true in any work item in the
   * cooperative group.
   */
  ...
  /**
   * @brief Evaluate predicate for all work items in the cooperative group and
   * returns an integer whose Nth bit is set if and only if predicate evaluates
   * to non-zero for the Nth work item.
   * @param pred The predicate to evaluate.
   * @return An integer whose Nth bit is set if and only if predicate evaluates
   * to non-zero for the Nth work item.
   */
  __device__ inline lane_mask ballot(int pred) const {
    auto result_ballot_sync = __ballot_sync(__group_mask, pred);
    result_ballot_sync =
        (__size == WAVEFRONT_SIZE)
            ? result_ballot_sync
  ...
  __device__ inline int thread_rank() const {
    ...
    int rank = (__size == WAVEFRONT_SIZE)
                   ? lane_id
                   : __popcll(__group_mask & ((1L << (lane_id)) - 1));
    // printf("mask: %llx lane_id: %d rank %d size %d\n", __group_mask, lane_id,
    // rank, __size);
    return rank;
  }
  ...
  /**
   * @brief Copies a variable from a source rank to all other ranks in the
   * cooperative group.
   *
   * @tparam T the type of the variable that should be shuffled.
   *
   * @param var The variable to broadcast (from source rank)
   * @param srcRank The rank in the cooperative group from which the variable is
   * broadcasted.
   *
   * @return The value of var from the work item with rank srcRank.
   */
  template <class T> __device__ inline T shfl(T var, int srcRank) const {
    int srcLane = (__size == WAVEFRONT_SIZE)
                      ? srcRank
                      : __fns64(__group_mask, 0, srcRank + 1);
    // printf("mask %llx rank: %ld lane: %ld\n", __group_mask, srcRank,
    // srcLane);
...
public:
  __device__ tiled_partition_internal_ext()
      : cooperative_group_base(Size, ~0) { // Include all threads
 ...
 * "If there exists a data-dependent conditional branch in the application code
 * such that threads within a warp diverge, then the warp serially executes each
 * branch disabling threads not on that path. The threads that remain active on
 * the path are referred to as coalesced." (Quote from:
...
public: // todo(hip): hide constructor
   ...
   * @param lm Lane mask in which bit N is set if and only if the Nth thread
   * belongs to the coalesced group.
   */
  __device__ coalesced_group_ext(lane_mask lm)
      : cooperative_group_base(__popcll(lm), lm) {
    set_mask(lm);
  }
};
 ...
 * grouped into the same group if and only if they have the same predicate
 * value.
 *
 * @param parent_tile The parent thread block tile to be partitioned.
 * @param pred The predicate to evaluate. Work items with the same predicate
 * value will be assigned to the same partition.
 */
template <uint32_t Size>
__device__ inline coalesced_group_ext
binary_partition(tiled_partition_internal_ext<Size> &parent_tile, bool pred) {
 ...
 *
 * @tparam Size The size of the thread block tile. CAUTION: Currently, only
 * cooperative groups of size <=64 are supported.
 */
template <uint32_t Size>
...
/**
 * @brief Creates a thread_block_tile from a parent cooperative group (HIP
 * implementation).
 *
 * @tparam Size The size of the thread block tile. CAUTION: Currently, only
 * cooperative groups of size <=64 are supported.
 *
 * @return The thread_block_tile cooperative group, which the calling work item
 * belongs to.
 */
template <uint32_t Size>
__device__ thread_block_tile<Size>
tiled_partition(cooperative_groups::thread_block tb) {
